...
void *turn_on(void *args) {
    ...
    toy->tempo_exec = rand() % 3 + 1; // Simulando tempo de execução aleatório
    ...
    pthread_mutex_init(&toy->mtx_cond, NULL);
    pthread_cond_init(&toy->cond, NULL);
    ...
    while (1) {
        // Esperando a condição ser satisfeita
        pthread_mutex_lock(&toy->mtx_cond);
        pthread_cond_wait(&toy->cond, &toy->mtx_cond);
...
void close_toys(){
    ...
    for (int i = 0; i < total_brinquedos; i++)
    {
        ...
        pthread_mutex_destroy(&brinquedos[i]->mtx_cond);
        pthread_cond_destroy(&brinquedos[i]->cond);
    }

    debug("[CLOSE] - Brinquedos encerrados\n");
